...
from utils.data_loader50 import CDNet2014Loader
...
if True:
    ...
    parser.add_argument('--recent_bg', metavar='Recent Background Frame', dest='recent_bg', type=int, default=50,
...
if True:
    ...
    transforms_tr = [
        ...
        [aug.NormalizeTensor(mean_rgb=mean_rgb, std_rgb=std_rgb,
                            mean_seg=mean_seg, std_seg=std_seg, segmentation_ch=0)]
    ]
    ...
    dataloader_tr = CDNet2014Loader(
                        dataset_tr, recent_win= recent_bg-1, transforms=transforms_tr,
    )
    dataloader_test = CDNet2014Loader(
                        dataset_test, recent_win= recent_bg-1, transforms=transforms_test,
    )
...
if network == "Resnet34":
    model = Thesis_Model(input_channels=recent_bg, output_channels=1)
else:
